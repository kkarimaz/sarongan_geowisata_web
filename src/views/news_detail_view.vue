  ...
  <div v-if="article">
    ...
    <section v-if="others.length" class="border-t border-brand-950/5 bg-brand-50/40">
      <div class="mx-auto max-w-7xl px-6 py-16 md:px-10">
        <h2 class="font-display text-2xl font-semibold text-brand-900">Berita Lainnya</h2>
        <div class="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          <ArticleCard v-for="item in others" :key="item.slug" :article="item" base-path="/news" />
        </div>
      </div>
    </section>
  </div>
